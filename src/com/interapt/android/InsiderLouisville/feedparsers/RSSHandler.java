package com.interapt.android.InsiderLouisville.feedparsers;
...
import com.interapt.android.InsiderLouisville.utils.CategoryUtils;
...
public class RSSHandler extends DefaultHandler {
	...
	static CategoryUtils categoryUtils = new CategoryUtils();
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		}else if (localName.equalsIgnoreCase("creator"))
		{
			...
			Log.d("LOGGING RSS XML", "Setting article dc:creator: " + chars.toString());
			currentArticle.setDc_creator(chars.toString());
		}
		else if (qName.equalsIgnoreCase("slash:comments"))
		{

			Log.d("LOGGING RSS XML", "Setting article slash:comments: " + chars.toString());
			try {
				int i = Integer.valueOf(chars.toString());
				currentArticle.setSlash_comments(i);
			} catch (NumberFormatException e) {
				// TODO Auto-generated catch block
				currentArticle.setSlash_comments(0);
				e.printStackTrace();
			}
		}else if (localName.equalsIgnoreCase("category"))
		{
			Log.d("LOGGING RSS XML", "Setting article content: " + chars.toString());
			currentArticle.getCategory().add(chars.toString());
			if(currentArticle.getArticleId()==0l)
			{
				currentArticle.setArticleId(categoryUtils.getCategoryLong(chars.toString()));
			}
		...
		}else if (localName.equalsIgnoreCase("commentRss"))
		{
			try {
				Log.d("LOGGING RSS XML", "Setting article link wfw:commentRss: " + chars.toString());
				currentArticle.setWfw_commentRss(new URL(chars.toString()));
			} catch (MalformedURLException e) {
				Log.e("RSA Error", e.getMessage());
			}
		...
		{
			try {
				Log.d("LOGGING RSS XML", "Setting article link guid: " + chars.toString());
				currentArticle.setGuid(new URL(chars.toString()));
			} catch (MalformedURLException e) {
				Log.e("RSA Error", e.getMessage());
			}
	 ...
	 * @return
	 */
	public ArrayList<News> getLatestArticles(String feedUrl) {
		URL url = null;
		try {
